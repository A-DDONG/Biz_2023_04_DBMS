...
-- TABLESPACE : 데이터를 저장할 물리적 저장소(파일), 데이터를 저장하기 위하여 가장 먼저 생성해야할 객체
-- USER : DBMS 서버에 로그인을 하고, 자신이 관리할 데이터들과 연결하는 객체
-- 오라클에서 USER 는 데이터 저장소의 개념이다. 이 개념은 다른 DBMS 와는 약간 다르게 취급한다
-- 저장소 SCHEME 라고 표현한다

-- C:/app/data : TALBESPACE 가 저장될 폴더
...
SIZE 1M AUTOEXTEND ON NEXT 1K; -- 초기의 저장소 공간을 1M(Byte) 확보하고 부족하면 1K(Byte)씩 자동으로 증가하라
...
-- 일부 프로그래밍 언어에서 DB 에 접속할때 ## 와 같은 특수문자가 있으면 접속에 문제를 일으키는 경우가 있다
-- 오라클에서는 12c 이후에 사용자 생성 정책을 예전 방식으로 사용할수 있도록 하는 설정을 제공한다

-- 이 설정 명령은 USER 를 생성하기 전에 항상 실행해 주어야 한다
