...
-- 현재 tbl_address TABLE 에 저장된 데이터가 모두(*) 몇개냐?
...
SELECT MAX(A_ID) FROM tbl_address;
SELECT MIN(A_ID) FROM tbl_address;
SELECT MAX(A_ID), MIN(A_ID) FROM tbl_address;

/*
SQL(Structure Query Language) : 구조화된 DBMS 핸들링 문법
...
-- 컬럼의 일부만 표현하도록 하는 방법 : Projection 이라고 한다
SELECT a_id, a_name
FROM tbl_address;
...
-- 모든 칼럼(*)으로 조회를 하면, 실제 원하는 칼럼의 순서와 관계없이
-- 데이터가 표현되기도 한다
SELECT * 
FROM tbl_address;

-- TABLS 의 모든 칼럼을 나열한 Projection
-- SELECT 명령에서 모든 칼럼(*) 으로 조회하는 것 보다는 ALL Project 로
-- 조회를 하는 것이, 조회 속도면에서 더 유리하다
...
PROJECTION : 특정칼럼만 나열하여 데이터를 SELECT 하는것
...
2. 테이블 있으면 데이터를 조회할 준비
3. WHERE 절의 조건대로 데이터 SELECTION
4. SELECT 결과 칼럼대로 데이터 PROJECTION
...
-- 전화번호 앞에서 6글자를 잘라서
-- 전화번호가 090-33 으로 시작되느 데이터 SELECTION
SELECT *
FROM tbl_address
...
조금더 깊이 생각하여 효율적인 SQL 명령문을 고민해보자

1. 첫번째 SELECTION
    먼저 범위를 제한하여 개수를 제한하고 전화번호를 찾는다
    
2. 두번째 SELECTION
    전체 범위에서 전화번호를 찾고 그 중에서 범위를 제한하여 개수를 제한한다
*/
SELECT * 
FROM tbl_address
...
SELECT * FROM tbl_address ORDER BY a_id;
