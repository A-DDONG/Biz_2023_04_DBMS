...
보안상 가장 위험한 사용자 생성
무결성 파괴
만약 이 사용자가 자신의 id 와 비번을 소흘히 하여 누군가에게 노출된다면
이 id 와 비번을 통하여 DB 에 접근하고 DBMS 데이터를 모두 파괴할 수 있다
*/
create USER 'todo'@'%'
IDENTIFIED BY '88888888';

-- 현재 등록된 사용자는 MySQL Server 에 접속할 수 있도록 권한을 가지고 있지만
-- 그외의 나머지 역할은 수행할 수 없다alter
-- DB 생성, Table 생성 등을 수행하려면 권한을 부여해야 한다
-- GRANT DBA TO user; 

-- ALL PRIVILEGES : DBA 권한
-- *.* : 모든 Database 에 대하면 모든 역할 수행
...
-- todoDB Database 의 tbl_todolist 테이블만 접근하는 권한 부여
GRANT ALL PRIVILEGES ON todoDB.tbl_todolist TO 'todo'@'192.168.4.%'; 

GRANT SELECT, INSERT, UPDATE, DELETE ON todoDB.tbl_todolist TO 'todo'@'192.168.4.%'; 
