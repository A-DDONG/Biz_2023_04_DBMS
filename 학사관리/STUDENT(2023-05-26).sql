...
-- DBA 권한은 테이블생성, 수정, 삭제, 데이터의 CRUD, COMMIT, ROLLBACK 등의 명령을 수행할 수 있다
...
-- 테이블의 구조를 변경하는 명령은 매우 비용이 크기 때문에 사용할때 항상 주의를 해야 한다
-- 특히 칼럼의 크기(길이)를 변경하거나 type 을 변경할때는 데이터 손상이 될 수 있기 때문에
-- 매우 조심해야 한다
...
-- 학생이름으로 정렬하고, 같은 이름이 있으면 그 범위내에서 전화번호로 한번더 정렬하여 보여라
SELECT st_num,st_name,st_dept,st_grade,st_tel
FROM tbl_student
...
-- 2. 조회된 데이터 중에서 변경하고자 하는 데이터의 PK 가 무엇인지 확인한다
...
-- 이몽룡(전화번호가 010-333-1111 인) 학생이 자퇴를 하고 이민을 갔다
...
-- 2. 삭제하고자 하는 데이터의 PK 를 확인
-- 여기서는 230003 인 PK 를 확인하였다
-- 3. PK 를 기준으로 삭제를 실행
DELETE FROM tbl_student
WHERE st_num = '230003';
