...
VALUES('0002','성춘향','010-222-2222','전라북도 남원시');
...
tbl_buyer 에 TABLE 에 저장된 데이터를 "아무 조건 없이"
모두 조회하여 보여달라
*/
...
-- 데이터가 없다 이런 경우 Result(결과) 가 Empty 인 상태 이다
...
buname 칼럼을 기준으로 조횐한 데이터가 비록 현재는 1개만 보이더라도
이 데이터는 0개 이상의 데이터가 조회될 것이다 라는 것을 반드시 예상해야 한다

만약 buname 칼럼을 기준으로 하여 데이터를 Update, Delete 를 수행할 경우
다수의(2개이상) 데이터에 변경(수정)이 이루어질 수 있다
그러한 이유로 Update, Delete 를 수행할 때는 절대 buname 칼럼을 기준으로 삼아서는 안된다
*/
...
/*
buid 칼럼을 기준으로 조회하기
buid 칼럼을 TABLE 을 create 할때 PK(PRIMARY KEY) 설정을 설정하였다
...
PK 칼럼에 0001 이라는 값을 갖는 데이터가 있는데 또다시 0001 이라는 데이터를 추가하려고 했기 때문에 발생하는 오류
*/
...
/*
TABLE 을 create 하면서 buid 와 buname 을 NOT NULL 로 설정하였다
데이터를 추가(INSERT)할 때 최소한 buid 와 buname 값은 반드시 필요하다라는 선언
...
이 단계에서는 TABLE 을 Entity 라고 한다

프로그래밍(Java 등) 에서는 TABLE 을 객체, 개체 등으로 취급한다
...
-- 만약 PK 가 아닌 칼럼을 기준으로 할때는 매우 신중하게 명령을 수행해야 한다
...
1. buname 칼럼의 데이터가 성춘향 인 리스트를 조회한다
...
/*
1. 전체 고객 데이터를 조회
2. 이몽룡의 주소가 현재 서울특별시이다 그런데 이몽룡이 전라북도 익산시로 이사를 했다
3. 이몽룡의 주소를 서울특별시에서 잔라북도 익산시로 변경하고자 한다
*/
SELECT * FROM tbl_buyer;
...
-- 데이터 수정
SELECT [TABLE] SET 칼럼 = 값 -- 변경할 변수와 깂
WHERE 칼럼 = 값              -- 변경할 조건
...
/*
현재 계좌정보를 조회했는데 고객정보가 고객ID 뿐이어서 고객에 대한 이름, 전화번호 등을 알수 없다
고객정보와 계좌정보를 연계하여 함께 볼수 있다면 좋겠다

Table JOIN
2개이상의 Table 을 서로 연계하여 하나의 리스트로 보기

tbl_acc 와 tbl_buyer 테이블을 연계하여 하나의 리스트로 보여라
이때 tbl_acc 의 acbuId 와 tbl_buyer 의 buid 칼럼의 데이터를 비교하여 같은 데이터는 한 라인에 보여라
*/
...
추가, 수정, 삭제된 정보는 아직 storage 에 반영되지 않고 메모리에 임시 보관(저장)된 산태이다
이 상태일때 Java 에서 SELECT 를 수행하면 INSERT, UPDATE, DELETE 된 데이터가 아닌 이전 상태듸 데이터가 조회된다
간혹 이 상황에서 DBMS 가 Connection 에서 무한정 응답하지 않는 경우도 있다
Java 는 DBMS 가 응답하기를 기다리면서 무한정 기다리고 마치 프로젝트가 멈춘(down)상태가 되어 버린다

SQD 에서 INSERT, UPDATE, DELETE 를 수행한 다음에는 강제로 storage 에 Commit 을 해주어야 한다
