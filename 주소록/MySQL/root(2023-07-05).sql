-- 이 화면은 root 화면
/*
MySQL 은 DataBase 라는 Scheme 를 통하며 물리적인 DB 를 관리한다
Oracle 은 TableSpace 라는 물리적 저장소와 연결한 사용자를 통하여 DB 를 관리한다

현업에서 가장 많이 사용하는 2가지의 DBMS 를 서로 비교하므로써 데이터베이스에 대한 이론을 정리해보기 바람

Oracle 에서는 사용자가 Scheme 가 되며 
MySQL 사용자는 DBMS, 물리적저장소 등에 접근하는 권한을 가진 사용자 개념이다

*/
-- 프로젝트 시작을 하기 위하여 Database 를 생성
...
BIGINT type 의 PK 칼럼
	...
	별도으 칼럼을 생성하고 그 칼럼에 일련번호 속성을 부여하여 PK로 만든다
    이때 PK 의 값은 무한히 커질수 있으므로 INT 형식보다 상당히 큰 BIGINT type 으로 설정한다
    
AUTO_INCREMENT 속성
	MySQL 에서는 일련번호 칼럼에 대하여 자동값 증가 생성 속성이 있다
    PK 칼럼에만 이 속성을 부여할수 있고
    이 속성을 가진 PK 칼럼은 Insert 가 수행될때 자동으로 1씩 증가하는
    일련번호를 생성하여 칼럼 데이터에 주입한다
*/
