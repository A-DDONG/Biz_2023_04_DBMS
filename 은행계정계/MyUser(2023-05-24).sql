...
-- 고객ID 가 0001 인 고객의 계좌정보를 잔액 순으로 정렬하여 보이기
-- 오름차순(ASCENDING) 정렬 : 기본값이므로 키워드 생략 가능
SELECT * FROM tbl_acc WHERE acbuid = '0001' ORDER BY acbalance ASC;

-- 고객 ID 가 0001 인 고객의 계좌정보를 잔액이 큰 순서대로 정렬하여 보이기
-- 내림차순(DESCENDING)
...
-- 각각 범위 값이 포함관계(>=, <=) 인 경우 BETWEEN 키워드로 검색 가능
...
-- tbl_acc 의 acNum 칼럼의 길이가 VARCHAR2(10) 인데 이 칼럼의 길이를 VARCHAR2(20) 으로 변경
...
-- substr(칼럼,시작,개수) : 칼럼의 문자열을 시작위치부터 개수만큼 잘라내기
-- max(칼럼) : 전체 리스트(조건에 맞는 데이터) 중에서 가장 큰 값을 찾기
-- min(칼럼) : 전체 리스트(조건에 맞는 데이터) 중에서 가장 작은 값 찾기
SELECT max(acNum) FROM tbl_acc WHERE substr(acNum,0,8) = '20230524';
