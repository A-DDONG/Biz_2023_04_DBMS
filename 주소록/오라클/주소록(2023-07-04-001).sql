...
이 테이블에 만약 4번째 취미를 추가하고 싶으면, 새로운 칼럼을 추가하거나 4번째 취미를 다른 취미 항목으로 교체해야 한다
만약 취미항목이 늘어나 칼럼을 추가하게 되면 Table 구조를 변경해야하고, 상당히 많은 비용이 요구된다(시간 등)
또한 Table 구조를 변경하는 것은 매우 신중하게 처리해야 한다  
다른 어플리케이션과 연동되는 Table 의 구조를 변경하면 아주 많은 부분에서 변경을 해야하는 경우도 있다

만약 취미항목이 3개 미만인 경우는 데이터가 저장되지 않은 칼럼이 발생하고 이 칼럼들은 보통 null 값으로 채워진다
...
ALTER : 객체변경, 사용중에 중요한 변경사항이 발생했을때 구조변경을 수행하는 명령, 구조변경은 매우 신중하게 수행해야 한다
*/


/*
주소 취미 Relation Table 의 제3정규화
제2정규화가 되어있는 tbl_addr_hobby TABLE 에는 주소록의 ID(a_id) 와 취미이름(hb_name)이 저장되어 있다
hb_aid 칼럼은 주소 테이블과 연계되어 있지만 hd_name 칼럼은 평범한 문자열로 구성되어 있다
...
UPDATE 명령은 기본적으로 PK 를 기준으로 1개의 데이터만 변경되도록 하는 것이 좋다. 하지만 이 명령은 이러한 원칙에 위배된다

이때 업데이트, 삭제를 수행하는 과정에서 이 테이블에는 수정이상, 삭제이상 현상이 발생할수 있다
또한 취미정보를 추가하는 과정에서 유사한 취미가 다른 형식으로 등록되는 경우도 있다

이러한 여러가지 문제를 방지하기 위하여 테이블을 분리하여 제3정규화를 수행한다

1. 기존의 테이블에서 일반문자열로 되어있는 부분을 떼어 새로운 테이블에 옮긴다
2. 기존 테이블과 새로운 테이블 간에 릴레이션을 설정한다



*/

-- 1. tbl_hobby talbe 의 이름변경 : Entity 를 Relation 으로 이름 변경
...
--    중복 없이 데이터를 모으기
-- hb_name 칼럼의 데이터를 같은 데이터끼리 그룹지어 리스트를 만든다
...
-- 3. 제3정규화 데이터 import
-- 주의! 기존 제2정규화 데이터를 엑셀로 보내서 제3정규화 데이터가 완료된 후 다음을 진행할것
-- tbl_addr_hobby table 을  기존 테이블을 drop 하고 새로운 구조로 생성하기
...
/*
Table JOIN
정규화(2,3정규화) 된 Table 의 정보를 Select(조회)하면 단순히 1개의 Table 만으로는 어떤 정보를 보여주고 있는지 알수가 없다
2,3 정규화된 Table 은 필연적으로 Table 간의 Join 이 되어야만 정보를 명확히 알 수 있다
JOIN 은 2개 이상의 Table 을 서로 연계하여 필요한 정보를 확인하는 것이다
*/
SELECT *
FROM tbl_addr_hobby, tbl_address;
 
-- tbl_addr_hobby 곱하기 tbl_address 카티션곱
SELECT COUNT(*)
FROM tbl_addr_hobby, tbl_address;

-- addr_hobby Relation 과 address Entity 테이블을 나열하되 두 테이블의 칼럼 ah_aid 와 a_id 의 데이터가 일치하는 정보만 나열하라
-- EQ JOIN
-- EQ JOIN 은 모든 JOIN 방식중에 가장 빠르다. 단 참조무결성 제약조건이 완벽해야 한다 
-- EQ JOIN 은 연계하는 Table 간에 참조무결성 제약조건이 완벽할때는 아무런 문제가 없이 결과를 조회할 수 있다
-- 하지만 연계하는 Table 간에 참조무결성 제약조건이 위배되면 조회된 데이터를 신뢰할 수 없다
...
-- LEFT JOIN
-- JOIN 을 실행하는 테이블 간에 참조무결성이 의심스럽거나, 위배될지 모른다는 전제하에 데이터를 확인하는 JOIN
-- FROM [table1] LEFT JOIN [table2]
-- 왼쪽(LEFT)의 table1 데이터는 무조건 SELECT 하고 오른쪽의 table2 데이터는 ON 에서 설정한 조건에 맞는 것만 가져오기
...
-- view 의 생성
-- 물리적 테이블에 SQL 을 적용하여 작성하였더니 다소 복잡한 SQL 이 완성되었다
...
CREATE VIEW view_addr_hobby
AS (        
    SELECT ah_aid AS 아이디, ah_hbcode AS 취미코드, hb_name AS 취미, a_name AS 이름, a_tel AS 전화번호, a_addr AS 주소
        FROM tbl_addr_hobby
            LEFT JOIN tbl_address
                ON a_id = ah_aid
            LEFT JOIN tbl_hobby
...
-- 연계된 Entity, Relation 간에 참조관계가 잘 유지되도록 테이블에 제약조건을 설정하는것
-- 1. FK 는 Relation 테이블에 설정한다
-- 2. FK 는 1:N 관계 테이블에서 N 의 테이블에 설정한다

-- addr hobby 와 address 간의 FK 설정
...
ALTER TABLE tbl_addr_hobby DROP CONSTRAINT f_addr;
ALTER TABLE tbl_addr_hobby DROP CONSTRAINT f_hobby;
/*
Table 의 FK 연계조건
-------------------------------------------------------
